...
def train(cfg):
    ...
    dataloader, stats = data_load(pred_horizon=pred_horizon, obs_horizon=obs_horizon, batch_size=batch_size,
    ...
    with tqdm(range(num_epochs), desc='Epoch') as tglobal:
        ...
        for epoch_idx in tglobal:
            ...
            for idx, nbatch in enumerate(dataloader):
                ...
                nimage = nbatch['image'][:, :obs_horizon].to(torch.float32).to(device)
                ...
                image_features2 = nets['vision_encoder'](
            ...
            if epoch_idx % 50 == 0:
                ...
                torch.save(nets.state_dict(), f'ckpts_pushT/rst_epoch{epoch_idx}_{np.mean(epoch_loss):.3f}.ckpt')
                torch.save(ema_nets.state_dict(), f'ckpts_pushT/ema/rst_epoch{epoch_idx}_{np.mean(epoch_loss):.3f}.ckpt')
...
cfg = omegaconf.OmegaConf.load('config/cfg_pushT.yaml')
train(cfg)
